...
void PostProcess::Apply() const
{
	...
	D3DApp::GetD3D11DeviceContext()->OMSetDepthStencilState(m_depth_stencil_state, 0);

	D3DApp::GetD3D11DeviceContext()->VSSetShader(m_vertex_shader, NULL, 0);
	D3DApp::GetD3D11DeviceContext()->GSSetShader(NULL, NULL, 0);
...
bool PostProcess::LoadPixelShaderFromFile(const tstring& file_name, const tstring& entry_point)
{
	...
	if (FAILED(hr))
	{
		...
		{
			std::string error_message = static_cast<char*>(error_buffer->GetBufferPointer());
			m_error_message = to_tstring(error_message);
		}
...
bool PostProcess::LoadPixelShaderFromMemory(const tstring& shader_content, const tstring& entry_point)
{
	...
	std::string shader_content_str = to_string(shader_content);
	HRESULT hr = D3DX11CompileFromMemory(
		shader_content_str.c_str(),
		shader_content_str.length(),
		NULL,
		NULL,
		NULL,
		to_string(entry_point).c_str(),
...
void PostProcess::InitVertexBuffer()
{
	...
	vertex_buffer_desc.Usage = D3D11_USAGE_DEFAULT;1,
...
void PostProcess::InitDepthStencilState()
{
	CD3D11_DEPTH_STENCIL_DESC ds_desc(D3D11_DEFAULT);
	ds_desc.DepthEnable = FALSE;
	D3DApp::GetD3D11Device()->CreateDepthStencilState(&ds_desc, &m_depth_stencil_state);
}
